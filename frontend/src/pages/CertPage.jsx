...
function SeverityPill({ text }) {
  const level = ["Critical", "High", "Medium", "Low"].find((l) => text?.includes(l));
  const cls = SEVERITY_COLOR[level] ?? "text-gray-400 bg-gray-800 border-gray-700";
  return level ? (
    <span className={`text-xs font-bold px-2 py-0.5 rounded border ${cls}`}>{level}</span>
  ) : null;
}
...
function loadConversations() {
  try { return JSON.parse(localStorage.getItem("cti_cert_conversations") || "[]"); }
  catch { return []; }
}

function saveConversations(convs) {
  localStorage.setItem("cti_cert_conversations", JSON.stringify(convs));
}
...
function UserBubble({ text }) {
  return (
    <div className="flex justify-end">
      <div className="max-w-[75%] bg-orange-700 text-white rounded-2xl rounded-br-sm px-4 py-3 text-sm leading-relaxed">
        {text}
      </div>
    </div>
  );
  ...
  if (loading) {
    return (
      <div className="flex justify-start">
        <div className="bg-gray-800 border border-orange-900 rounded-2xl rounded-bl-sm px-4 py-3">
      ...
      <div className="max-w-[80%] space-y-2">
        ...
        {sources && sources.length > 0 && (
          <div>
            ...
            >
              {showSources ? "Hide" : "Show"} {sources.length} advisor{sources.length !== 1 ? "ies" : "y"}
            </button>
            {showSources && (
              <div className="mt-2 space-y-2">
                {sources.map((s) => (
                  <div key={s.chunk_id} className="flex items-start gap-2">
                    ...
                    <div className="flex-1"><ResultCard {...s} /></div>
                  </div>
                ))}
              </div>
            )}
    ...
    <aside className="w-64 flex-shrink-0 bg-gray-900 border-r border-gray-700 flex flex-col h-full">
      ...
      <div className="p-3 border-t border-gray-700">
        <p className="text-xs text-gray-600 text-center">
          CERT · {conversations.length} session{conversations.length !== 1 ? "s" : ""}
        </p>
      </div>
    </aside>
  );
...
export default function CertPage() {
  ...
  const [activeId, setActiveId] = useState(() => {
    ...
    const savedId = localStorage.getItem("cti_cert_active_id");
  ...
  useEffect(() => {
    if (activeId) localStorage.setItem("cti_cert_active_id", activeId);
    else localStorage.removeItem("cti_cert_active_id");
  }, [activeId]);
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {messages.length === 0 && !loading && (
          <div className="flex-1 flex flex-col items-center justify-center text-center gap-4 px-4">
            ...
            <p className="text-gray-400 text-sm">Search Microsoft security advisories and CVE records</p>
            <div className="flex flex-wrap gap-2 justify-center mt-2">
              {SUGGESTIONS.map((s) => (
                <button key={s} onClick={() => handleSend(s)}
                  className="text-xs px-3 py-1 rounded-full border border-orange-800 text-orange-400 hover:bg-orange-900/40 transition-colors">
                  {s}
                </button>
              ))}
            </div>
        ...
        <div className="px-4 py-4 max-w-4xl w-full mx-auto">
          ...
          <div className="flex gap-2 items-end bg-gray-800 border border-gray-700 rounded-2xl px-4 py-3 focus-within:border-orange-500 transition-colors">
            <textarea
              value={input}
            ...
            <button onClick={() => handleSend()} disabled={loading || !input.trim()}
              className="flex-shrink-0 px-4 py-1.5 bg-orange-600 hover:bg-orange-700 disabled:bg-gray-700 disabled:text-gray-500 text-white rounded-xl font-medium text-sm transition-colors">
              {loading ? "..." : "Send"}
            </button>
          </div>
        </div>
